	...
	BEGIN
		...
		BEGIN
			...
			CREATE TABLE dbo.fhsmCPUUtilization(
				...
				,CONSTRAINT PK_CPUUtilization PRIMARY KEY(Id)
			);

			CREATE NONCLUSTERED INDEX NC_fhsmCPUUtilization_EventTimeUTC ON dbo.fhsmCPUUtilization(EventTimeUTC);
			CREATE NONCLUSTERED INDEX NC_fhsmCPUUtilization_EventTime ON dbo.fhsmCPUUtilization(EventTime);
		END;
		...
		BEGIN
			...
			CREATE TABLE dbo.fhsmCPUPerDatabase(
				...
				,TimestampUTC datetime NOT NULL
				,Timestamp datetime NOT NULL
				,CONSTRAINT PK_CPUPerDatabase PRIMARY KEY(Id)
			);

			CREATE NONCLUSTERED INDEX NC_fhsmCPUPerDatabase_TimestampUTC ON dbo.fhsmCPUPerDatabase(TimestampUTC);
			CREATE NONCLUSTERED INDEX NC_fhsmCPUPerDatabase_Timestamp ON dbo.fhsmCPUPerDatabase(Timestamp);
		END;
		...
		BEGIN
			...
			SET @stmt = '
				...
				SELECT
					...
					,cpd.CPUPercent
					,cpd.Timestamp
					,CAST(cpd.Timestamp AS date) AS Date
					,(DATEPART(HOUR, cpd.Timestamp) * 60 * 60) + (DATEPART(MINUTE, cpd.Timestamp) * 60) + (DATEPART(SECOND, cpd.Timestamp)) AS TimeKey
					,(SELECT k.[Key] FROM dbo.fhsmFNGenerateKey(cpd.DatabaseName, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT) AS k) AS DatabaseKey
